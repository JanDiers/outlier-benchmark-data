...
@dataclass
class Pima(BaseDataset):
    """
    The Pima dataset has the following properties:
    ...
    No further preprocessing has been applied. The file is named semantic/Pima/Pima_withoutdupl_05_v01.arff in the
    collection of [1]_.

    Usage:

    >>> from outlier_benchmark.datasets import pima
    >>> X, y = pima.load(download=True)  # download will only take place if not previously downloaded
    >>> X.shape  # (526, 8)
    >>> y.sum()  # 26, the number of outliers in the dataset
    >>> X.max()  # 744.0
    >>> X.min()  # 0.0
    ...
    name: str = field(default='Pima', init=False)
