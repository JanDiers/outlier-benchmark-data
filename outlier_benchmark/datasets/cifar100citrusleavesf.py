...
@dataclass
class Cifar100CitrusLeavesF(BaseDataset):
    """
    The Cifar100CitrusLeavesF dataset has the following properties:
    ...
    >>> from outlier_benchmark.datasets import cifar100citrusleavesf
    >>> X, y = cifar100citrusleavesf.load(download=True)  # download will only take place if not previously downloaded
    >>> X.shape
    (594, 320)
    ...
    name: str = field(default='Cifar100CitrusLeavesF', init=False)
...
cifar100citrusleavesf = Cifar100CitrusLeavesF()
